...
export const MobileBottomNav = ({ onOpenMission, missionCount = 0 }: MobileBottomNavProps) => {
    ...
    return (
        <>
            ...
            <nav className="fixed bottom-0 left-0 right-0 h-20 bg-slate-950/80 backdrop-blur-2xl border-t border-white/5 z-50 lg:hidden flex items-start justify-around px-4 pt-3 pb-[env(safe-area-inset-bottom,12px)]">
                {menuItems.map((item) => {
                    if (item.special) {
                        return (
                            <button
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                "relative flex flex-col items-center justify-center gap-0.5 xs:gap-1 flex-1 max-w-[4rem] h-12 transition-all active:scale-90 touch-manipulation",
