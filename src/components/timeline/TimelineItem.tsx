    ...
    return (
        ...
        >
            ...
            <div className={`relative p-4 sm:p-8 rounded-2xl bg-gradient-to-br ${config.bgGradient} backdrop-blur-xl border ${config.borderColor} shadow-xl ${config.shadowColor} hover:shadow-2xl hover:scale-[1.01] transition-all duration-300 overflow-hidden ${onClick ? 'cursor-pointer' : ''}`}>
                ...
                <div className="relative flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 sm:mb-8 gap-3 sm:gap-4">
                    <div className="flex items-center gap-3 sm:gap-4">
                        {/* Icon */}
                        <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl ${config.iconBg} border ${config.borderColor} flex items-center justify-center shadow-lg ${config.shadowColor} group-hover:scale-110 transition-transform duration-300 flex-shrink-0`}>
                            <Icon className={`w-6 h-6 sm:w-7 sm:h-7 ${config.textColor}`} />
                        </div>

                        {/* Badge */}
                        <div className={`px-3 py-1.5 sm:px-5 sm:py-2 rounded-xl bg-gradient-to-r ${config.gradient} text-white shadow-lg font-bold text-xs sm:text-sm uppercase tracking-wider`}>
                            {config.badge}
                        </div>
                    </div>

                    {/* Time */}
                    <div className="flex items-center gap-2 sm:gap-3 text-slate-400 ml-auto sm:ml-0">
                        <Clock className="w-4 h-4 sm:w-5 sm:h-5" />
                        <span className="font-semibold text-sm sm:text-base">{formatTime(item.timestamp)}</span>
                    </div>
                </div>


                {/* Título da Atividade */}
                <h3 className={`text-xl sm:text-2xl font-bold ${config.textColor} mb-4 sm:mb-5 line-clamp-2 leading-tight break-words`}>
                    {displayTitle}
                </h3>

                {/* Descrição */}
                {item.description && (
                    <p className="text-slate-300 text-sm sm:text-base mb-6 sm:mb-8 line-clamp-2 leading-relaxed break-words">
                        {item.description}
                    </p>
                )}
